// basic
// import Demo from '@containers/basic/01_组件通信';
// import Demo from '@containers/basic/02_生命周期';
// import Demo from '@containers/basic/03_dangerouslySetInnerHTML';
// import Demo from '@containers/basic/04_propTypes';
// import Demo from '@containers/basic/05_context';
// import Demo from '@containers/basic/06_ref';
// import Demo from '@containers/basic/07_高阶组件';
// import Demo from '@containers/basic/08_错误边界';
// import Demo from '@containers/basic/09_renderProps';

// hooks
// import Demo from '@containers/hooks/demo';
// import Demo from '@containers/hooks/useState';
// import Demo from '@containers/hooks/useEffect';
// import Demo from '@containers/hooks/useMemo';
// import Demo from '@containers/hooks/useCallback';
// import Demo from '@containers/hooks/useContext';
// import Demo from '@containers/hooks/useReducer';
// import Demo from '@containers/hooks/useRef';
// import Demo from '@containers/hooks/useLogger';

// css
// import Demo from '@containers/css/01_css';
// import Demo from '@containers/css/02_sass';
import Demo from '@containers/css/03_css-modules';
// import Demo from '@containers/css/04_styled-containers';
// import Demo from '@containers/css';

// redux
// import Demo from '@/containers/redux/redux-core';
// import Demo from '@containers/redux/redux-core/bindActionCreators';
// import Demo from '@containers/redux/redux-core/compose';
// import Demo from '@containers/redux/react-redux';
// import Demo from '@containers/redux/react-redux/hooks';
// import Demo from '@containers/redux/middleware/diy';
// import Demo from '@/containers/redux/middleware/redux-thunk';
// import Demo from '@containers/redux/middleware/redux-saga';

// router
// import Demo from '@/containers/router/01_基础用法';
// import Demo from '@containers/router/02_路由传参';
// import Demo from '@containers/router/03_Switch';
// import Demo from '@containers/router/04_Redirect';
// import Demo from '@containers/router/05_路由嵌套';
// import Demo from '@containers/router/06_withRouter';
// import Demo from '@containers/router/07_react-router-config';
// import Demo from '@containers/router/09_Route';
// import Demo from '@containers/router/10_Link';

function App() {
  return (
    <div className='App'>
      <Demo />
    </div>
  );
}

export default App;
